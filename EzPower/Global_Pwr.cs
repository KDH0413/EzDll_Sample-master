...
namespace EzPower
{
    ...
    public class Global_Pwr
    {
        public enum ePWRTYPE
        {
            TabosBattery
            , ElgenBattery
            , ElgenCharger
        ...
        public class PWRSTATUS
        {
            public int soc { get; set; }
            public double curr { get; set; }
            public double volt { get; set; }
            public double temp_1st { get; set; }
            public double temp_2nd { get; set; }
            public double temp_3th { get; set; }
            public double temp_4th { get; set; }
            public int errbit { get; set; }
            public ePWR_CHR_STATE state { get; set; }

            public PWRSTATUS()
            {
                curr = volt = 0;
                soc = 0;
                temp_1st = temp_2nd = temp_3th = temp_4th = 0;
                errbit = 0;
                state = ePWR_CHR_STATE.None;
            }
        }

        public class JCVT_PWR
        {
            public string typeName { get; set; }
            public string json { get; set; }
            public static JCVT_PWR Set<T>(T data)
            {
                var obj = new JCVT_PWR();
                obj.typeName = $"{typeof(T).Name}";
                obj.json = JsonConvert.SerializeObject(data, Formatting.Indented);
                return obj;
            }

            public static void Get(string data, out PWRSTATUS arg)
            {
                arg = JsonConvert.DeserializeObject<PWRSTATUS>(data);
            }
        }
    }
